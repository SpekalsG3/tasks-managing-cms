...
formSubmits[0].onclick = function(e) {
  e.preventDefault();
  if (formInputs[0].value == selected[0].children[0].innerHTML &&
    formInputs[1].value == selected[1].children[0].innerHTML &&
  ...
  if (req == "")
    return;
  ajax({
    url: this.parentNode.action + "?action=" + (prevWasAdd ? "add" : "edit&query=" + selectedRow) + req + "&taskedited=" + (formInputs[2].value == selected[2].children[0].innerHTML ? "false" : "true"),
    method: "GET",
    callback: function(response) {
